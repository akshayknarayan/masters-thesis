...
\subsection{Differences in Algorithm Implementations}
\label{s:ccp:deficit}

Because CCP algorithms operate asynchronously, the semantics of their implementations can vary from traditional datapath-native ones. 
One example of this difference is the implementation of traditional congestion avoidance-style algorithms (\ie most algorithms proposed thus far), which consist of congestion window increase and decrease rules upon incoming acknowledgements and indications of congestion -- usually, packet loss. 
These algorithms must take care to only invoke their congestion window reduction logic \emph{once} per window of packets because it is unfair to perform multiple reductions when multiple packets in the same window are lost: after the first loss in the window, the algorithm has no opportunity to reduce its congestion window to prevent subsequent losses within the same window. 

However, unlike datapath-native algorithms, CCP algorithms do not know explicitly which packets belong to the same window. Therefore, they must infer from available primitives when it is appropriate to perform further window reductions. Intuitively, a single window reduction period lasts from the first indication of a loss until all the packets that were inflight at that time are acknowledged. 
Therefore, CCP algorithms can count the number of out-of-order packets received along with a notification of loss, and wait until the cumulative acknowledgement covers those packets. Once this condition is met, the algorithm can resume performing congestion window reductions on subsequent loss reports.
